...
public class Eksempel3 {
    ...
    public static void main(String[] args) {
        ...
        Collections.sort(People.people, (a,b) -> a.lastName().compareTo(b.lastName()));

        //2 - Ofte ser vi kode som dette
        Collections.sort(People.people, comparing(p -> p.lastName()));

        //3 - Eller (enda bedre) ved å bruke metodereferanse
        Collections.sort(People.people, comparing(Person::lastName));
...
//		Collections.sort(people, vedAaSammenligne(Person::age));
        Collections.sort(People.people, vedAaSammenligne(Person::firstName));
        People.people.forEach(System.out::println);
    }


    static <T, U extends Comparable<? super U>> Comparator<T> vedAaSammenligne(Function<T,U> xtractor) {
